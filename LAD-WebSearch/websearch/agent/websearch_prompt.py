...
- The response angle needs to revolve around meme culture or image metaphors. Please pay special attention to the relevant search results.
"""
...
8. 问题拆分需要围绕梗文化或图片隐喻，请注意精确检索相关话题内容
"""
...
8. The refinement of the question needs to focus on meme culture or image metaphors. Please pay attention to accurately searching for relevant topic content.
"""
...
graph.add_root_node(node_content="疯狂星期四V我50是什么梗?", node_name="root") # 添加原始问题作为根节点
graph.add_node(
        node_name="疯狂星期四", # 节点名称最好有意义
        node_content="疯狂星期四是什么活动？")
graph.add_node(
        node_name="V我50", # 节点名称最好有意义
        node_content="V我50的具体含义？")
graph.add_node(
        node_name="sub_name_3", # 节点名称最好有意义
        node_content="content of sub_name_3")
...
graph.add_edge(start_node="root", end_node="sub_name_1")
...
graph.node("疯狂星期四"), graph.node("V我50"), graph.node("sub_name_3"), ...
```<|action_end|>
"""
...
<|action_start|><|interpreter|>```python
graph = WebSearchGraph()
graph.add_root_node(node_content="What is the meaning of Crazy Thursday Give Me 50?", node_name="root") # Add the original question as the root node
graph.add_node(
        node_name="Crazy Thursday", # The node name should be meaningful
        node_content="What is Crazy Thursday?")
graph.add_node(
        node_name="Give Me 50", # The node name should be meaningful
        node_content="What is the specific meaning of Give Me 50 dollars?")
graph.add_node(
        node_name="sub_name_3", # The node name should be meaningful
        node_content="content of sub_name_3")        
...
graph.add_edge(start_node="root", end_node="sub_name_1")
...
# Get node info
graph.node("Crazy Thursday"), graph.node("Give Me 50"), graph.node("sub_name_3"), ...
```<|action_end|>
"""
...
- The answer should be based on meme culture or image metaphors.
